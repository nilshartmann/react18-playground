  ...
  app.use((req, _res, next) => {
    ...
    if (token) {
      ...
      if (!user) {
        return res
          .status(401)
          .json({ error: `User with id '${payload.userId}' not found` });
      }
      req.user = user;
      req.userId = user.id;
    }
...
app.get("/posts", (req, res) => {
  ...
  if (req.query.short !== undefined) {
    result = datastore
      .getAllPosts()
      .map((p) => ({ id: p.id, date: p.date, title: p.title }));
...
app.get("/posts/:id/comments", (req, res) => {
  setTimeout(() => {
    const comments = datastore.getCommentsForPost(req.params.id);
...
app.post("/posts", (req, res) => {
  if (authEnabled && !req.user) {
    return res
      .status(401)
      .json({ error: "You must be logged in to execute this action" });
  }
...
app.put("/posts", (req, res) => {
  if (authEnabled && !req.user) {
    return res
      .status(401)
      .json({ error: "use must be logged in to execute this action" });
  }
